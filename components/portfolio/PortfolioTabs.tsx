...
export default function PortfolioTabs({
    ...
    return (
        <div className={className}>
            <div className="flex overflow-x-auto hide-scrollbar border-b border-neutral-200 dark:border-neutral-700 mb-6">
                {items.map((item) => (
                    <button
                        key={item.id}
                        onClick={() => setActiveTab(item.id)}
                        className={`
              relative px-4 py-2 mr-4 font-medium text-sm transition-all
              ${activeTab === item.id
