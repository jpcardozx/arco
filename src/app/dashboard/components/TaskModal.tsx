...
import { Task, Client } from '@/lib/types/supabase-helpers'
import { safeSplit } from '@/lib/types/helpers'
import { ClientsServiceWrapper, TasksServiceWrapper } from '@/lib/services/client-wrapper'
import { useCurrentUser } from '@/lib/hooks/useCurrentUser'
import { ShareModal } from '@/components/shared/ShareModal'

type SystemUser = { id: string; name: string; email: string; role: string; department?: string; full_name?: string }
...
export function TaskModal({ isOpen, onClose, onSave, task }: TaskModalProps) {
    ...
    const [formData, setFormData] = useState({
        ...
        property_id: '',
        due_date: '',
        task_type: 'task' as 'task' | 'event' | 'reminder' | 'meeting',
    ...
    const loadClients = async () => {
        ...
        try {
            const data = await ClientsServiceWrapper.getClients()
            setClients(data)
        } catch (error) {
    ...
    const loadUsers = async () => {
        ...
        try {
            // TODO: Implement UserService when needed
            // For now, use mock data or just the current user
            if (user) {
                const userName = (user as any).name || user.user_metadata?.full_name || user.email?.split('@')[0] || 'Usuário'
                setUsers([{ id: user.id, name: userName, email: user.email ?? '', role: 'user' }])
            } else {
                setUsers([])
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (task) {
            setFormData({
                ...
                task_type: (task.task_type as 'task' | 'event' | 'reminder' | 'meeting') || 'task',
                visibility: (task.visibility as 'private' | 'team' | 'public') || 'private',
                category: task.category || '',
                start_time: task.start_time || '',
                end_time: task.end_time || '',
                reminders: Array.isArray(task.reminders) ? (task.reminders as any[]).map(r => String(r)) : [],
                notifications_enabled: true
            })
        } else {
            setFormData({
    ...
    const validateForm = () => {
        ...
        if (formData.client_id && formData.client_id.trim() !== '' && !formData.client_id) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (task?.id) {
                await TasksServiceWrapper.updateTask(task.id, taskData as any)
            } else {
                await TasksServiceWrapper.createTask(taskData as any)
            }
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => setFormData({ ...formData, task_type: e.target.value as 'task' | 'event' | 'reminder' | 'meeting' })}
                                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                            >
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                {formData.task_type === 'task' && 'Tarefa geral ou administrativa'}
                                                {formData.task_type === 'event' && 'Evento ou compromisso'}
                                                {formData.task_type === 'meeting' && 'Reunião agendada'}
                                                {formData.task_type === 'reminder' && 'Lembrete simples'}
                                            </p>
                                        </div>
                                                    ...
                                                    <div className="space-y-1">
                                                        ...
                                                        <button
                                                            ...
                                                            onClick={async () => {
                                                                ...
                                                                console.log('Debug: Database access')
                                                            }}
