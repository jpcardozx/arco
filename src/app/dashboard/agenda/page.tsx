...
export default function AgendaPage() {
    ...
    const loadAgendaStats = async () => {
        ...
        try {
            ...
            // Filter by assigned_to if user exists
            ...
            if (userTasks) {
                ...
                const todayEvents = userTasks.filter(task => {
                    if (!task.due_date) return false
                    const taskDate = new Date(task.due_date)
                    return taskDate.toDateString() === today.toDateString()
                }).length

                const weekEvents = userTasks.filter(task => {
                    if (!task.due_date) return false
                    const taskDate = new Date(task.due_date)
                    return taskDate >= startOfWeek && taskDate <= endOfWeek
                }).length

                const monthEvents = userTasks.filter(task => {
                    if (!task.due_date) return false
                    const taskDate = new Date(task.due_date)
                    return taskDate >= startOfMonth && taskDate <= endOfMonth
                }).length

                const overdueEvents = userTasks.filter(task => {
