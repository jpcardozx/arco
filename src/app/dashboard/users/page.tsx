...
// import { RBACManager } from '@/lib/auth/password-authorization'
...
export default function UsersPage() {
    ...
    useEffect(() => {
        ...
        if (state.searchTerm) {
            ...
            filtered = filtered.filter(user =>
                (user as any).full_name?.toLowerCase().includes(search) ||
                user.email.toLowerCase().includes(search) ||
                (user as any).department?.toLowerCase().includes(search)
            )
        }
    ...
    const canChangePassword = async (targetUser: UserProfile) => {
        ...
        return hasPermission && ((targetUser as any).role?.hierarchy_level || 0) < ((currentUser as any).role?.hierarchy_level || 100)
    }
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {state.filteredUsers.map((user) => (
                                <tr key={user.id} className="hover:bg-gray-50">
                                        ...
                                        <div className="flex items-center">
                                            <div className="flex-shrink-0 h-10 w-10">
                                                <div className="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white font-medium">
                                                    {((user as any).full_name || user.name || 'U').charAt(0).toUpperCase()}
                                                </div>
                                            </div>
                                            <div className="ml-4">
                                                <div className="text-sm font-medium text-gray-900">
                                                    {(user as any).full_name || user.name || 'Usuário'}
                                                </div>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getRoleColor((user as any).role?.hierarchy_level || 50)}`}>
                                            {(user as any).role?.name || user.role}
                                        </span>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {(user as any).last_login || user.lastLogin ? new Date((user as any).last_login || user.lastLogin).toLocaleDateString('pt-BR') : 'Nunca'}
                                    </td>
                    ...
                    <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-semibold">
                                Alterar Senha - {(state.selectedUser as any).full_name || state.selectedUser.name}
                            </h2>
                            <button
                ...
                <div className="bg-white rounded-lg border p-4">
                    <div className="flex items-center">
                        ...
                        <div className="ml-3">
                            ...
                            <p className="text-lg font-semibold text-gray-900">
                                {state.users.filter(u => ((u as any).role?.hierarchy_level || 0) >= 90).length}
                            </p>
                        </div>
                    </div>
                </div>
