...
function FeatureSectionComponent({
  ...
  const renderFeature = (feature: Feature, index: number) => {
    ...
    const renderIcon = () => {
      if (typeof feature.icon === 'string') {
        ...
        return (
          <div
            className={cn(
              'flex items-center justify-center rounded-lg bg-blue-50 dark:bg-blue-900/30',
              getIconSizeClass()
            )}
          >
            <Image
        ...
        return (
          ...
          >
            <Check
              className={iconSize === 'lg' ? 'h-8 w-8' : iconSize === 'md' ? 'h-6 w-6' : 'h-5 w-5'}
            />
          </div>
        );
      }
    ...
    if (layout === 'alternating') {
      return (
        <motion.div key={index} className={cn('py-10', baseFeatureClasses)} {...animationProps}>
    ...
    return (
      <motion.div key={index} className={baseFeatureClasses} {...animationProps}>
        <div className="flex flex-col gap-4">
          {feature.badge && (
            <Badge variant="outline" size="sm">
              {feature.badge}
  ...
  return (
    ...
    >
      <div
        className={cn('container mx-auto px-4', align === 'center' ? 'text-center' : 'text-left')}
      >
        {/* Section header */}
        {(displayTitle || displaySubtitle || displayDescription) && (
          <div className={cn('mb-12 max-w-3xl', align === 'center' ? 'mx-auto' : '')}>
            {displaySubtitle && (
            ...
            {displayDescription && (
              <p className="text-lg text-neutral-600 dark:text-neutral-400">{displayDescription}</p>
            )}
          </div>
        )}
        ...
        {/* Action link */}
        {showActionLink && (
          <div className={cn('mt-12', align === 'center' ? 'text-center' : 'text-left')}>
            <a
              href={actionLinkHref}
