...
export function AnimatedSection({
    ...
    if (animationVariant === 'stagger') {
      return React.Children.map(children, (child, i) => {
        if (React.isValidElement(child)) {
          return (
            <motion.div key={i} variants={childVariants} custom={i}>
              {child}
            </motion.div>
          );
        }
  ...
  // Extract className from sectionProps to avoid type error
  const { className, ...restSectionProps } = sectionProps;

  return (
    <Section {...restSectionProps} className={cn('overflow-hidden', className)}>
      <motion.div
        ref={ref}
        initial="hidden"
        animate={controls}
