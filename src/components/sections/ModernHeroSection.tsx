    ...
    return (
        <section className="section-hero relative overflow-hidden">
            ...
            <div className="absolute inset-0 opacity-30">
                ...
                <div className="absolute bottom-20 right-20 w-40 h-40 bg-accent-200 rounded-full blur-xl" />
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-gradient-to-r from-primary-100 to-accent-100 rounded-full blur-3xl opacity-50" />
            </div>
            ...
            <div className="container-custom relative z-10">
                ...
                >
                    ...
                    {/* Subheadline */}
                    <motion.p
                        className="text-xl md:text-2xl text-neutral-600 mb-6 max-w-4xl mx-auto font-light leading-relaxed"
                        variants={itemVariants}
                    >
                    ...
                    {/* Emergency CTA */}
                    <motion.div
                        className="inline-flex items-center gap-3 bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 rounded-2xl px-8 py-4 mb-12 group hover:shadow-medium transition-all duration-300"
                        variants={itemVariants}
                    >
                        ...
                        <span className="badge badge-warning ml-2">24/7</span>
                    </motion.div>
                    ...
                    >
                        {trustIndicators.map((indicator, index) => (
                            ...
                            >
                                <div className="p-3 rounded-xl bg-gradient-to-br from-white to-neutral-50 shadow-soft group-hover:shadow-medium transition-all duration-300 group-hover:-translate-y-1">
                                    <indicator.icon className={`w-5 h-5 ${indicator.color}`} />
                                </div>
                                <span className="font-semibold text-neutral-700 group-hover:text-neutral-900 transition-colors">{indicator.text}</span>
                            </div>
                        ))}
                    </motion.div>
                    ...
                    >
                        {socialProof.map((proof, index) => (
                            <motion.div
                                ...
                                whileHover={{ y: -4 }}
                            >
                                <div className="text-2xl font-bold text-gradient mb-2">{proof.metric}</div>
                                <div className="text-sm font-medium text-neutral-700 mb-1">{proof.company}</div>
                ...
                >
                    {stats.map((stat, index) => (
                        <motion.div
                            key={index}
                            className="card-stat group cursor-default"
                            variants={itemVariants}
